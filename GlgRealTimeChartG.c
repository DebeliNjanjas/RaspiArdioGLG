...
#define NUM_PLOTS 1           /* Number of lines in a chart. */
...
void UpdateChart( GlgAnyType data, GlgIntervalID * id ,double value);
...
void GetChartData( double time_stamp, GlgBoolean use_current_time, double value );
...
void GetChartData( double time_stamp, GlgBoolean use_current_time,double value )
{
   ...
   for( i=0; i<NUM_PLOTS; ++i )
   {
	 ...
	 supply application specific data for a given plot index.
        */
     // vred= GlgImportStrings( Data, "program.txt" );	
...
void PreFillChartData()
{
   ...
   {      
      ...
      GetChartData( time_stamp, False, value );
   }
}
